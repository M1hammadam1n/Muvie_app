...
import 'package:move_app_1/domain/blocs/top_tv_bloc/toptvshow_bloc.dart';
...
class TvShowTopRated extends StatelessWidget {
  const TvShowTopRated({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final locale = Localizations.localeOf(context);
    return BlocProvider(
      create: (_) => TopTvShowListBloc(const TopTvShowListState.inital())
        ..add(TopTvShowListEventLoadReset())
        ..add(TopTvShowListEventLoadNextPage(locale.languageCode)),
...
class _TvShowListWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<TopTvShowListBloc, TopTvShowListState>(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              if (index >= state.tvshow.length - 1) {
                ...
                context.read<TopTvShowListBloc>().add(TopTvShowListEventLoadNextPage(locale));
              }
