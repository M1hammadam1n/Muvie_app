...
import 'package:move_app_1/domain/api_client/api_client_exception.dart';
import 'package:move_app_1/domain/blocs/auth_bloc/auth_bloc.dart';

abstract class AuthViewCubitState {}

class AuthViewCubitSuccessFormFillInProgressState extends AuthViewCubitState {
  @override
...
class AuthViewCubit extends Cubit<AuthViewCubitState> {
  ...
  void auth({
    ...
    if (!_isValid(login, password)) {
      final state = AuthViewCubitErrorState(
  ...
  void _onState(AuthState state) {
    if (state is AuthUnauthorizedState) {
      emit(AuthViewCubitSuccessFormFillInProgressState());
    } else if (state is AuthAuthorizedState) {
      authBlocSubscription.cancel();
      emit(AuthViewCubitSuccessAuthState());
    } else if (state is AuthFailureState) {
      final message = _mapErrorToMassage(state.error);
  ...
  String _mapErrorToMassage(Object error) {
